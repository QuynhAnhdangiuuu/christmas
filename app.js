const audio = document.createElement('audio');
    audio.src = 'https://files.catbox.moe/8mvjay.mp3'; // Đường dẫn file nhạc
    audio.autoplay = true; // Tự động phát
    audio.loop = true; // Phát lại liên tục
    audio.style.display = 'none'; // Ẩn trình phát
...
$(document).ready(function () {
  ...
  let textTitle = "Gửi Nhồi!";
  let charArrTitle = textTitle.split("");
  let text =
    "T không chắc mình là người đặc biệt trong lòng m, nhưng với t, m luôn là người đặc biệt nhất. Chúc m có một Giáng Sinh thật vui vẻ!";
  ...
  function resetText() {
    ...
    // Xóa ảnh khi reset
    const imgElement = document.querySelector(".cart-page-bottom img");
    if (imgElement) {
      imgElement.remove();
    }
  }
        ...
        if (currentIndexTitle < charArrTitle.length) {
          ...
          textIntervalContent = setInterval(function () {
            if (currentIndexContent < charArrContent.length) {
              ...
              // Thêm ảnh sau khi text chạy xong
              const img = document.createElement("img");
              img.src = "emxinh.jfif"; // Thay bằng đường dẫn ảnh của bạn
              img.alt = "Description"; // Mô tả ảnh
              img.style.marginTop = "10px"; // Tùy chỉnh khoảng cách
              img.style.width = "200px"; // Đặt chiều rộng ảnh nhỏ hơn
              img.style.height = "auto"; // Tự động điều chỉnh chiều cao theo tỉ lệ
              cartPageBottomP.appendChild(img);
            }
          }, 100);
          $(".cart-page-bottom").append(img); // Thêm ảnh vào phần tử
              img.fadeIn(10000); // Hiệu ứng fade in ảnh trong 1 giây
